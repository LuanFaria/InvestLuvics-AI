...
# --- FUNÇÃO DE HIGIENIZAÇÃO DE VALORES (CONVERTE '50,52' PARA 50.52) ---
def para_float(val):
...
def limpar_dataframe_numerico(
  ...
  """Garante que todas as colunas de valores sejam float puro para evitar erros no PyArrow."""
  if df.empty:
    return df
  df_limpo = df.copy()
    ...
    for tag in soup.find_all(["h3", "span", "p", "small", "b", "td", "dt"]):
      ...
      if termo.lower() == texto.lower() or (
        ...
        for _ in range(4):
          ...
          if not parent:
            break
          valor_tag = parent.find("strong", class_="value") or parent.find(
              "strong"
          )
          if valor_tag:
            val = valor_tag.get_text(strip=True)
...
def atualizar_cotacoes():
    ...
    for _, row in fundos_df.iterrows():
      ...
      try:
        ...
        if response.status_code == 200:
          ...
          val_txt = extrair_indicador(
              soup, ["valor atual", "valor atual do ativo"]
          )
  ...
  if atualizados_total > 0:
    st.success(
        f"Sucesso! {atualizados_total} ativos (FIIs e Ações) atualizados no"
        " banco de dados."
    )
  else:
    st.warning(
        "⚠️ Nenhum ativo foi atualizado. Verifique se os tickers estão corretos"
...
with st.sidebar:
  st.markdown(
      "<h2 style='text-align: center; color: #4DA8DA; letter-spacing: 2px;'>TECH"
      " LUVICS</h2>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<p style='text-align: center; color: #888; margin-top: -10px;'>Painel de"
      " Investimentos</p>",
      unsafe_allow_html=True,
  )
  st.markdown("---")
...
if menu == "Dashboard":
  ...
  try:
    controle_raw = pd.read_sql(
        "SELECT * FROM controle ORDER BY CAST(ano AS INTEGER) DESC, CAST(mes"
        " AS INTEGER) DESC",
        conn,
    )
  ...
  # Sanitização rigorosa das tabelas
  ...
  # Cálculos dos Totais
  caixa_atual = (
      float(controle_df["caixa"].iloc[0])
      if not controle_df.empty and "caixa" in controle_df.columns
      else 0.0
  )
  tot_fundos = (
      float(fundos_df["valor_mercado"].sum())
      if not fundos_df.empty and "valor_mercado" in fundos_df.columns
      else 0.0
  )
  tot_acoes = (
      float(acoes_df["valor_mercado"].sum())
      if not acoes_df.empty and "valor_mercado" in acoes_df.columns
      else 0.0
  )
  tot_fixa = (
      float(fixa_df["total_investido"].sum())
  ...
  m1.metric(
      "Caixa Atual",
      f"R$ {caixa_atual:,.2f}".replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  m2.metric(
      "Total FIIs",
      f"R$ {tot_fundos:,.2f}".replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  m3.metric(
      "Total Ações",
      f"R$ {tot_acoes:,.2f}".replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  m4.metric(
      "Total Renda Fixa",
      f"R$ {tot_fixa:,.2f}".replace(",", "X")
      .replace(".", ",")
      .replace("X", "."),
  )
  ...
  if tipo_grafico == "Divisão da Carteira":
    ...
    if not fundos_ativos.empty:
      fig = px.pie(
          fundos_ativos, names="ticker", values="valor_mercado", hole=0.4
      )

  elif tipo_grafico == "Por Tipo de Fundo" and not fundos_df.empty:
    df_grp = fundos_df.groupby("segmento", as_index=False)[
        "valor_mercado"
    ].sum()
    ...
    if not acoes_ativas.empty:
      fig = px.pie(
          acoes_ativas, names="ticker", values="valor_mercado", hole=0.4
      )
    ...
    if not fixa_ativas.empty:
      fig = px.pie(
          fixa_ativas, names="ticker", values="total_investido", hole=0.4
      )
  ...
  with st.expander("🔍 Inspecionar Registros da Tabela de Controle Mensal"):
    if not controle_df.empty:
      ...
      default_meses = (
          ...
          else (meses_disponiveis[:1] if len(meses_disponiveis) > 0 else [])
      )
      ...
      colunas_numericas = [
          col for col in df_filtrado.columns if col not in colunas_ignorar
      ]

      col_c1, col_c2 = st.columns([1, 2])
      with col_c1:
        col_selecionada = st.selectbox(
            "Selecione a coluna para somar:", options=colunas_numericas
        )

      total_selecionado = (
          df_filtrado[col_selecionada].sum() if col_selecionada else 0.0
      )

      with col_c2:
        st.metric(
            f"Total"
            f" {str(col_selecionada).replace('_', ' ').capitalize() if col_selecionada else ''}",
            f"R$ {total_selecionado:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", "."),
        )
...
# ==============================================================================
  ...
  with st.form("form_controle"):
    ...
    with col1:
      ano = st.number_input(
          "Ano", min_value=2000, max_value=2100, value=int(hoje.year)
      )
    with col2:
      mes = st.number_input(
          "Mês", min_value=1, max_value=12, value=int(hoje.month)
      )
    ...
    with col3:
      ...
      coluna_alvo = st.selectbox(
          "Adicionar valor a qual coluna?", opcoes_colunas
      )
    with col4:
      valor_adicionar = st.number_input(
          "Valor a adicionar (R$)", value=0.0, step=50.0
      )

    st.info(
        "💡 **Dica:** O valor informado será somado à coluna selecionada para o"
        " mês escolhido."
    )
    ...
    if btn_salvar:
      ...
      cursor.execute(
          "SELECT * FROM controle WHERE ano = ? AND mes = ?",
          (int(ano), int(mes)),
      )
      ...
      cursor.execute(
          "SELECT caixa FROM controle WHERE ano = ? AND mes = ?",
          (int(ano_ant), int(mes_ant)),
      )
      row_ant = cursor.fetchone()
      caixa_anterior = para_float(row_ant[0]) if row_ant else 0.0
      ...
      if registro_id:
        ...
        st.success(
            f"✅ Lançamento computado no mês {mes}/{ano}! Categoria"
            f" '{coluna_alvo}' recebeu + R$ {valor_adicionar:.2f}."
        )
      else:
        cursor.execute(
            """
        ...
        st.success(
            f"✅ Mês {mes}/{ano} gerado e lançamento inserido com sucesso!"
        )
...
# ==============================================================================
  ...
  try:
    ...
    if "num_cotas" in df_fundos.columns and "cota_atual" in df_fundos.columns:
      df_fundos.insert(
          loc=3,
          column="Valor de Mercado (R$)",
          value=(df_fundos["num_cotas"] * df_fundos["cota_atual"]),
      )
  ...
  try:
    ...
    if "num_cotas" in df_acoes.columns and "cota_atual" in df_acoes.columns:
      df_acoes.insert(
          loc=3,
          column="Valor de Mercado (R$)",
          value=(df_acoes["num_cotas"] * df_acoes["cota_atual"]),
      )
...
# ==============================================================================
  ...
  tab1, tab2 = st.tabs(
      ["Adicionar Novo Ativo", "Lançar Operação (Compra/Venda)"]
  )
  conn = get_connection()

  with tab1:
    tipo_ativo_novo = st.selectbox(
        "Selecione a Categoria para Cadastro",
        ["FIIs", "Ações", "Renda Fixa"],
        key="cat_novo",
    )
      ...
      if st.form_submit_button("Cadastrar Ativo", width="stretch"):
        if nome:
          try:
            ...
            if tipo_ativo_novo == "FIIs":
              conn.execute(
                  "INSERT INTO fundos (ticker, segmento, num_cotas,"
                  " total_investido, cota_atual, pvp, dy) VALUES (?, ?, 0, 0, 0,"
                  " 0, 0)",
                  (ticker_limpo, tipo_fundo),
              )
            elif tipo_ativo_novo == "Ações":
              conn.execute(
                  "INSERT INTO acoes (ticker, num_cotas, total_investido,"
                  " cota_atual, dy) VALUES (?, 0, 0, 0, 0)",
                  (ticker_limpo,),
              )
            else:
              conn.execute(
                  "INSERT INTO renda_fixa (ticker, total_investido, dy) VALUES"
                  " (?, 0, 0)",
                  (ticker_limpo,),
              )
            conn.commit()
  ...
  with tab2:
    tipo_ativo_op = st.selectbox(
        "Selecione a Categoria para Operar",
        ["FIIs", "Ações", "Renda Fixa"],
        key="cat_op",
    )
    tabela = (
        ...
        else ("acoes" if tipo_ativo_op == "Ações" else "renda_fixa")
    )
    ...
    if df_ativos.empty:
      ...
      with st.form("form_op"):
        ...
        if tipo_ativo_op in ["FIIs", "Ações"]:
          qtd = st.number_input(
              "Quantidade de Cotas", min_value=0.01, step=1.0
          )
        else:
          val_total = st.number_input(
              "Valor Transacionado (R$)", min_value=1.0, step=100.0
          )
          qtd = 1.0
        ...
        if st.form_submit_button("Salvar Operação", width="stretch"):
          ...
          try:
            if tipo_ativo_op in ["FIIs", "Ações"]:
              cursor.execute(
                  f"SELECT cota_atual, num_cotas, total_investido FROM"
                  f" {tabela} WHERE ticker = ?",
                  (ativo_selecionado,),
              )
              res = cursor.fetchone()
              ...
              novas_cotas = max(
                  0.0, num_cotas_atuais + (float(qtd) * multiplicador)
              )
              novo_tot_investido = (
                  tot_investido_atual + valor_total
                  if multiplicador == 1
                  else max(0.0, tot_investido_atual - valor_total)
              )

              cursor.execute(
                  f"UPDATE {tabela} SET num_cotas = ?, total_investido = ?"
                  " WHERE ticker = ?",
                  (novas_cotas, novo_tot_investido, ativo_selecionado),
              )

              cursor.execute(
                  "INSERT INTO operacoes (ticker, data_op, tipo, quantidade,"
              ...
              cursor.execute(
                  "SELECT total_investido FROM renda_fixa WHERE ticker = ?",
                  (ativo_selecionado,),
              )
              res = cursor.fetchone()
              ...
              novo_tot_investido = max(
                  0.0, tot_investido_atual + (valor_total * multiplicador)
              )

              cursor.execute(
                  "UPDATE renda_fixa SET total_investido = ? WHERE ticker = ?",
                  (novo_tot_investido, ativo_selecionado),
              )
...
# ==============================================================================
  ...
  with col1:
    v_inicial = st.number_input(
        "Aporte Inicial (R$)", min_value=0.0, value=1000.0, step=100.0
    )
    v_mensal = st.number_input(
        "Aporte Mensal (R$)", min_value=0.0, value=500.0, step=50.0
    )
  with col2:
    taxa_anual = st.number_input(
        "Taxa de Juros Anual (%)", min_value=0.0, value=10.0, step=0.5
    )
    anos = st.number_input("Período (Anos)", min_value=1, value=10, step=1)

  taxa_mensal = ((1 + taxa_anual / 100) ** (1 / 12)) - 1
  meses = int(anos * 12)

  total_investido = v_inicial + (v_mensal * meses)
  saldo = v_inicial
  for _ in range(meses):
    saldo = (saldo + v_mensal) * (1 + taxa_mensal)

  juros = saldo - total_investido

  m1, m2, m3 = st.columns(3)
  m1.metric("Total Investido", f"R$ {total_investido:,.2f}")
  m2.metric("Total de Juros", f"R$ {juros:,.2f}")
  m3.metric("Valor Final Estimado", f"R$ {saldo:,.2f}")
